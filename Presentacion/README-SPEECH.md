...
El baseline mayoritario siempre predice maintain. Con 20 maintain y 8 review obtiene una accuracy de 0,7143, pero su Macro-F1 es 0,4167, porque no reconoce casos review. Con la evaluación vigente de 30 semillas, el árbol en perfil operational_core obtuvo Macro-F1 promedio 0,5987 con IC95 [0,5365; 0,6609]. Hoy el árbol aporta auditabilidad; su valor predictivo adicional deberá demostrarse con más fuentes, etiquetas expertas independientes y casos que H6 no resuelva por sí solo.

Se realizaron treinta holdouts estratificados con semillas diferentes. En cada repetición se utilizaron 21 casos para entrenamiento y 7 para prueba. El árbol aprendió una separación dominada por assertions_failed, y warning_count puede actuar como proxy. Por eso reportamos explícitamente el riesgo de sobreajuste: las variantes con proxies alcanzan Macro-F1 1,0000, mientras operational_core cae a 0,5987. Como las etiquetas provienen del motor H6, el resultado mide fidelidad respecto de ese motor, no superioridad frente al criterio experto ni generalización operacional.

## Slide 9 — Evaluación del modelo · 1:15

“La conclusión principal no es que el modelo sea perfecto. La evaluación mide fidelidad a las etiquetas de H6 dentro de una muestra pequeña. El baseline mayoritario quedó en Macro-F1 0,4167 y el perfil operational_core del árbol en 0,5987, pero la comparación ocurre solamente sobre 28 ejecuciones, distribuidas en 20 maintain y 8 review.

En 30 evaluaciones con semillas diferentes y separación estratificada, operational_core mostró desviación y rango amplios (std 0,1738; min 0,3000; max 0,8444), lo que es más realista que una métrica perfecta. Al mismo tiempo, las variantes con proxies conservaron Macro-F1 1,0000. La brecha promedio all_features - operational_core fue 0,4013 con IC95 [0,3391; 0,4635], clasificada como evidencia strong. Esto respalda la advertencia metodológica: hoy el ML sirve para auditar y controlar, no para afirmar generalización operacional.
